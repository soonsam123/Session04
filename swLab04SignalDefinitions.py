...
# reverse from python was not working.
def reverse(p):
    list = []
...
def polyR(s, p):
    ...
    i = 0
    for coeff in coeffs:
        # Storing the signals s Scaled by the coeff and delayed by the degree of the polynom.
        # The degree starts as 0 for the first element (which is the last of the polynom, that why it is reversed)
        # and end ups in the last element of the list (len(list) - 1), which turns out to be the first element of the polynom.
        list_signals.append(Rn(ScaledSignal(s, coeff), i))
        i = i + 1
    # Sum each signal two by two. (using __add__ propertie)
